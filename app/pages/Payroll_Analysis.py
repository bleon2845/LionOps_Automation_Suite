...
import services.payroll_rules as rule
...
def load_dev_data() -> dict:
    ...
    ausentismo_temporales = DEV_DATA_PATH / "Inf Ausentismo HE RN Consolidado 2025.xlsx"
    ...
    return {
        ...
        "ausentismo_temporales": load_excel(ausentismo_temporales, sheet="Ausentismo", name="Inf Ausentismo HE RN Consolidado 2025"),
    }
...
if DEV_MODE:
    ...
    df_infHe = dfs["ausentismo_temporales"]
    ...
    with col1:
        ...
        f_ausentismo_temporales = st.file_uploader("Inf Ausentismo HE RN Consolidado 2025.xlsx", type=["xlsx"])
    ...
    with st.spinner("Loading and validating files..."):
        ...
        df_infHe = load_excel(f_ausentismo_temporales, sheet="Ausentismo", name="Inf Ausentismo HE RN Consolidado 2025")
...
with st.spinner("Validating columns..."):
    ...
    check_columns(df_infHe, ["Identificacion"], "Inf Ausentismo HE RN Consolidado 2025")
...
periodos = rule.calcular_periodos_nomina(df_acumulado)
...
df_acumulado = rule.check_quantity_with_salary(
    df=df_acumulado,
...
df_acumulado = rule.calculate_for_concept(
    df=df_acumulado,
...
# --------------- COMPARATIVE OF NOMINA Y INTERFAZ ---------------
# Crear tablas resumen (equivalente a pivot_table sum)
df_summ_cifrasMes = rule.pivot_sum(
...
df_summ_acumulado = rule.pivot_sum(
    df=df_acumulado,
    index_cols=["NUMERO DOCUMENTO", "NOMBRE COMPLETO", "CONCEPTO"],
    value_col="NETO",
    output_col="NETO"
...
df_summ_acumulado = rule.total_by_group_first_row(
...
df_acumulado = rule.execute_analysis_days_payroll(
    df=df_acumulado,
...
# Execute Funciotn to Validate Offboarding Friday/Saturday + dto/no lab
df_acumulado = rule.validate_offboarding_weekdays(df_acumulado, empresa_value="SUPPLA S.A")

# Get number of days in current month based on rules and assign it to a variable
days_current_month = rule.config_days_month_rules(rule.get_days_month(nombreMesActual, anio_actual),base=30)

# Execute Function to Validate Change of Vinculation
df_acumulado = rule.validate_vinculation_change(df_acumulado, dias_mes_actual=days_current_month)

# Execute Function charge/salary validation vs previous month (merge + column)
df_acumulado = rule.validate_salary_role_previous_month(df_acumulado, df_consoNomina)
...
df_acumulado = rule.validate_days_by_novedades(df_acumulado,dias_mes_actual=days_current_month,empresa_value="SUPPLA S.A")
...
df_acumulado = rule.build_prev_month_days_columns(
...
days_prev_month = rule.config_days_month_rules(rule.get_days_month(nomMesAnterior, anio_actual), base=30)
...
df_acumulado = rule.validate_prev_month_vacations(
    df_acumulado,
...
df_acumulado = rule.validate_prev_month_unpaid_days_for_new_hires(
    df_acumulado,
    nom_mes_anterior=nomMesAnterior,
    dias_mes_actual=days_current_month,
    empresa_value="SUPPLA S.A",
...
# ---------------- ANALYSIS TEMPORARY STAFF PREVIOUS MONTH ----------------
# Filter File for Month
df_temporales_raw = filter_dataframe(df_infHe, column="Mes", values=nMesAnterior, case_insensitive=False)
df_temporalesActual_raw = filter_dataframe(df_infHe, column="Mes", values=nMesActual, case_insensitive=False)

# Dictionary for Create Columns
mapping = {
...
df_temporales = rule.build_temporales_summary(
    df_infhe=df_infHe,
    mes_value=nMesAnterior,
    mapping_novedad_to_col=mapping,
    col_mes="Mes",
    group_col="Identificacion",
    col_novedad="Novedad",
    col_semana="Sem",
    special_inasistencia="Inasistencia injustificada",
    fill_value_none=True,  # deja None cuando 0, igual a tu lógica
)

df_temporalesActual = rule.build_temporales_summary(
    df_infhe=df_infHe,
    mes_value=nMesActual,
    mapping_novedad_to_col=mapping,
    col_mes="Mes",
    group_col="Identificacion",
    col_novedad="Novedad",
    col_semana="Sem",
    special_inasistencia="Inasistencia injustificada",
    fill_value_none=True,
)

# Delete Duplicates
df_temporales = col.delete_duplicate_rows(df_temporales, column_name="Identificacion", keep="first")
df_temporalesActual = col.delete_duplicate_rows(df_temporalesActual, column_name="Identificacion", keep="first")

# Delete Columns
cols_drop = [
    "Mes","Ciudad","Operación","Concatenado","Sem","Fecha","Nombre","Cargo",
    "Contrato","Novedad","Gestionable","Gross?"
]
df_temporales = col.delete_columns(df_temporales, cols_drop)
df_temporalesActual = col.delete_columns(df_temporalesActual, cols_drop)

# Merge
df_acumulado = merge_dataframes(
    df_left=df_acumulado,
    df_right=df_temporales,
    left_key="NUMERO DOCUMENTO",
    right_key="Identificacion",
    how="left",
    merge_name="Acumulado vs Temporales Mes Anterior"
)

# ---------------- ANALYSIS TEMPORARY STAFF ABSENCES SICK LEAVE PREV MONTH ----------------
df_acumulado = rule.execute_temporales_validations(
    df_acumulado=df_acumulado,
    df_consoNomina=df_consoNomina,
    df_personalNacional=df_personalNacional,
    dias_mes_actual=days_current_month,
    nom_mes_anterior=nomMesAnterior,
    salario_umbral_pt=salary_target,   # el SMLV/base que ya tienes
)


# Delete Columns
cols_to_drop = [
    "FECHA DE RETIRO", "ID", "SALARIO BASICO", "CARGO NOMINA", "CEDULA",
...
df_acumulado = col.delete_columns(df_acumulado, cols_to_drop)

# Reset Index
df_temporalesActual = col.reset_index_if_named(df_temporalesActual, "Identificacion", drop=True)
df_acumulado = col.reset_index_if_named(df_acumulado, "Identificacion", drop=True)

# Merge
df_acumulado = merge_dataframes(
    df_left=df_acumulado,
    df_right=df_temporalesActual,
    left_key="NUMERO DOCUMENTO",
    right_key="Identificacion",
    how="left",
    merge_name="Acumulado vs Temporales Mes Actual",
)

# ------------------ REGLA TEMPORALES PT CON INCAPACIDADES (MES ACTUAL) ------------------
df_acumulado = rule.apply_temporales_incapacity_rule(
    df_acumulado,
    dias_mes_actual=days_current_month,
    salario_base=salary_target,   # tu umbral "salario" (SMLV / base)
)

# Create Column
df_acumulado = col.create_sum_column(
    df_acumulado,
    new_column="DIAS VACACIONES",
    columns_to_sum=["DIAS_VACACIONES", "DIAS_VACACIONES_FESTIVAS"],
    as_int=True,
)
