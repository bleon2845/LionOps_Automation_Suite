...
from services.columns import delete_columns, rename_columns, change_to_datetime
...
def validate_vinculation_change(
    ...
    *,
    ...
    # without group_keys=False to advoid errors
    out = out.groupby(group_col, group_keys=False).apply(_apply_group)
    if group_col not in out.columns and group_col in out.index.names:
        out = out.reset_index()
...
def build_temporales_summary(
    df_infhe: pd.DataFrame,
    mes_value,
    mapping_novedad_to_col: dict,
    *,
    col_mes: str = "Mes",
    group_col: str = "Identificacion",
    col_novedad: str = "Novedad",
    col_semana: str = "Sem",
    special_inasistencia: str = "Inasistencia injustificada",
    fill_value_none: bool = True,
) -> pd.DataFrame:
    """
    Construye un resumen por Identificacion (1 fila por persona) con columnas
    definidas en mapping_novedad_to_col contando ocurrencias por 'Novedad'.

    Regla especial:
    - Para 'Inasistencia injustificada': total = count + nunique(Sem)

    Parámetros:
    - df_infhe: DataFrame origen
    - mes_value: mes a filtrar (ej: nMesAnterior)
    - mapping_novedad_to_col: {"Vacaciones": "DIAS_VACACIONES_TEMP", ...}
    - fill_value_none: si True -> deja None cuando el valor es 0 (como tu script), si False -> 0
    """

    if df_infhe is None or df_infhe.empty:
        # devolver df vacío con estructura mínima
        out = pd.DataFrame(columns=[group_col] + list(mapping_novedad_to_col.values()))
        return out

    required = [col_mes, group_col, col_novedad]
    missing = [c for c in required if c not in df_infhe.columns]
    if missing:
        raise ValueError(f"Missing required columns in df_infhe: {missing}")

    df = df_infhe.copy()

    # Filtrar por mes
    df = df[df[col_mes] == mes_value]
    if df.empty:
        out = pd.DataFrame(columns=[group_col] + list(mapping_novedad_to_col.values()))
        return out

    # Asegurar que group_col no sea nulo
    df = df[df[group_col].notna()]
    if df.empty:
        out = pd.DataFrame(columns=[group_col] + list(mapping_novedad_to_col.values()))
        return out

    # Conteos por (Identificacion, Novedad)
    counts = (
        df.groupby([group_col, col_novedad])
        .size()
        .unstack(fill_value=0)
    )

    # Regla especial: Inasistencia injustificada = conteo + semanas únicas
    if special_inasistencia in counts.columns:
        if col_semana not in df.columns:
            # Si no existe Sem, no podemos sumar semanas; se deja solo conteo
            weeks_unique = pd.Series(0, index=counts.index)
        else:
            weeks_unique = (
                df[df[col_novedad] == special_inasistencia]
                .groupby(group_col)[col_semana]
                .nunique()
                .reindex(counts.index)
                .fillna(0)
                .astype(int)
            )
        counts[special_inasistencia] = counts[special_inasistencia].astype(int) + weeks_unique.astype(int)

    # Armar salida con columnas finales (según mapping)
    out = pd.DataFrame(index=counts.index)

    for desc, out_col in mapping_novedad_to_col.items():
        if desc in counts.columns:
            out[out_col] = counts[desc].astype(int)
        else:
            out[out_col] = 0

    # Si quieres replicar tu comportamiento de dejar None en lugar de 0
    if fill_value_none:
        out = out.where(out != 0, None)

    out = out.reset_index().rename(columns={group_col: "Identificacion"})
    return out

def execute_temporales_validations(
    df_acumulado: pd.DataFrame,
    df_consoNomina: pd.DataFrame,
    df_personalNacional: pd.DataFrame,
    *,
    dias_mes_actual: int,
    nom_mes_anterior: str,
    salario_umbral_pt: float,
    col_status: str = "ESTATUS_DIAS",
    col_obs: str = "OBSERVACION",
    col_empresa: str = "EMPRESA",
    col_doc: str = "NUMERO DOCUMENTO",
) -> pd.DataFrame:
    """
    Orquestador estándar (1 función grande) para:
    - Validar temporales por sumas (inasistencias / incapacidades) => OK + OBS
    - Reglas por retiro (nota crédito, pagos en liquidación)
    - Merge con nómina anterior (df_consoNomina) y validar ingreso mes anterior sin pago
    - Validar temporales PT con incapacidad => OK + OBS + DIAS_PAGO_NOMINA = dias_mes_actual
    - Merge con base nacional (df_personalNacional) y ajustar FECHA DE BAJA vs FECHA DE RETIRO
    - Recalcular DIFERENCIA_DIAS y validar retiro mismo mes (2 variantes)
    - Renombrar DIAS_INASISTENCIA_INJUSTIFICADA_TEMP => DIAS_INASISTENCIAS_TEMPORALES_MESANTERIOR
    """
    ...
    # ----------------- Helpers internos -----------------
    def _ensure_numeric_columns(df: pd.DataFrame, cols: List[str], default: float = 0) -> pd.DataFrame:
        df = df.copy()
        for c in cols:
            if c not in df.columns:
                df[c] = default
    ...
    def _ensure_text_columns(df: pd.DataFrame, cols: List[str], default: str = "") -> pd.DataFrame:
        df = df.copy()
        for c in cols:
            if c not in df.columns:
                df[c] = default
    ...
    def _set_status_obs(
        ...
        *,
        ...
        obs: Optional[pd.Series | str] = None,
    ) -> pd.DataFrame:
        df = df.copy()
        if status is not None:
            df.loc[mask, col_status] = status
        if obs is not None:
            df.loc[mask, col_obs] = obs
        return df

    def _update_status_by_sum_days(
        df: pd.DataFrame,
        *,
        base_mask: pd.Series,
        sum_cols: List[str],
        ...
        df = _ensure_numeric_columns(df, sum_cols, default=0)
        df = _ensure_text_columns(df, [col_status, col_obs, col_empresa], default="")

        suma = df[sum_cols].sum(axis=1)
        ok_mask = base_mask & (suma == dias_mes_actual)

        def _build_obs_row(row) -> str:
            parts = []
            for c in sum_cols:
                v = row.get(c, 0)
                if pd.notna(v) and float(v) > 0:
                    parts.append(f"{int(v)} {labels.get(c, c)}")
            return " + ".join(parts)

        obs_series = df.loc[ok_mask, sum_cols].apply(_build_obs_row, axis=1)

        df.loc[ok_mask, col_status] = "OK"
        df.loc[ok_mask, col_obs] = obs_series
        return df

    def _add_month_name_from_date(df: pd.DataFrame, *, date_col: str, out_col: str) -> pd.DataFrame:
        df = df.copy()
        df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
        df[out_col] = df[date_col].dt.month.map(lambda m: MESES[m - 1] if pd.notna(m) else None)
        return df

    def _validate_temporales_pt_incapacidad(df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
        df = _ensure_numeric_columns(
            df,
            ["SALARIO MENSUAL", "DIAS_INCAPACIDAD_E.G_TEMP", "DIAS_INCAPACIDAD_A.T_TEMP", "DIAS_PAGO_NOMINA"],
            default=0,
        )
        df = _ensure_text_columns(df, [col_status, col_obs, col_empresa], default="")

        mask = (
            (df[col_status] == "VALIDAR")
            & (df[col_empresa] != "SUPPLA S.A")
            & (df["SALARIO MENSUAL"] < float(salario_umbral_pt))
            & ((df["DIAS_INCAPACIDAD_E.G_TEMP"] > 0) | (df["DIAS_INCAPACIDAD_A.T_TEMP"] > 0))
        )

        suma_incap = (
            (df.loc[mask, "DIAS_INCAPACIDAD_E.G_TEMP"].fillna(0) + df.loc[mask, "DIAS_INCAPACIDAD_A.T_TEMP"].fillna(0))
            .astype(int)
            .astype(str)
        )

        obs = suma_incap + " Días Incapacidad, Pagos al 100%"
        df = _set_status_obs(df, mask, status="OK", obs=obs)
        df.loc[mask, "DIAS_PAGO_NOMINA"] = dias_mes_actual
        return df

    # ----------------------------------------------------
    # 1) Base mask temporales
    out = _ensure_text_columns(out, [col_status, col_obs, col_empresa], default="")
    base_mask_temporales = (out[col_status] == "VALIDAR") & (out[col_empresa] != "SUPPLA S.A")

    # 2) Validar temporales (inasistencias)
    columnas_inas = [
        "DIAS_PAGO_NOMINA",
    ...
    out = _update_status_by_sum_days(out, base_mask=base_mask_temporales, sum_cols=columnas_inas, labels=etiquetas_inas)

    # 3) Validar temporales (incapacidades)
    columnas_incap = ["DIAS_PAGO_NOMINA", "DIAS_INCAPACIDAD_E.G_TEMP", "DIAS_INCAPACIDAD_A.T_TEMP"]
    etiquetas_incap = {
        "DIAS_PAGO_NOMINA": "Dias Trabajados",
        "DIAS_INCAPACIDAD_E.G_TEMP": "Dias Incapacidad E.G",
        "DIAS_INCAPACIDAD_A.T_TEMP": "Dias Incapacidad A.T",
    }
    out = _update_status_by_sum_days(out, base_mask=base_mask_temporales, sum_cols=columnas_incap, labels=etiquetas_incap)

    # 4) Reglas: retiro temporales con días negativos / 0
    out = _ensure_numeric_columns(out, ["DIAS_PAGO_NOMINA"], default=0)

    mask_nc_retiro = (
        (out[col_status] == "VALIDAR")
        & (out[col_empresa] != "SUPPLA S.A")
        & (out[col_obs] == "Validar Retiro")
        & (out["DIAS_PAGO_NOMINA"] < 0)
    )
    out = _set_status_obs(out, mask_nc_retiro, status="OK", obs="Nota Crédito del Mes de Retiro")
    ...
    out = _set_status_obs(out, mask_pago_liq, status="OK", obs="Pagos Pendientes en Liquidación")

    # 5) Merge con nómina anterior
    out = merge_dataframes(
        ...
        right_key="CEDULA",
        how="left",
        merge_name="Acumulado vs ConsoNomina",
    )

    # 6) MES_INGRESO desde FECHA DE INGRESO
    out = _add_month_name_from_date(out, date_col="FECHA DE INGRESO", out_col="MES_INGRESO")

    # 7) Temporales con ingreso mes anterior y sin pago (CEDULA vacía)
    mask_ingreso_mes_anterior_sin_pago = (
        (out[col_status] == "VALIDAR")
        & (out[col_empresa] != "SUPPLA S.A")
        & (out["MES_INGRESO"] == nom_mes_anterior)
        & (out["CEDULA"].isna() | (out["CEDULA"] == ""))
    )
    out = _set_status_obs(out, mask_ingreso_mes_anterior_sin_pago, status="OK", obs="Se Facturan Dias Desde Su Fecha de Ingreso")

    # 8) Temporales PT con incapacidades mes anterior
    out = _validate_temporales_pt_incapacidad(out)

    # 9) Merge con base nacional (copia sin columnas)
    df_personal_copy = delete_columns(df_personalNacional, ["CARGO NÓMINA", "FECHA DE INGRESO"])

    out = merge_dataframes(
        df_left=out,
        df_right=df_personal_copy,
        left_key=col_doc,
        right_key="ID",
        how="left",
        merge_name="Acumulado vs Copia Base Personal Nacional",
    )

    # 10) Ajustar FECHA DE BAJA vs FECHA DE RETIRO
    out = change_to_datetime(out, "FECHA DE BAJA", error="coerce")
    out = change_to_datetime(out, "FECHA DE RETIRO", error="coerce")

    mask_update_baja = (
        (out[col_status] == "VALIDAR")
        & (out[col_empresa] != "SUPPLA S.A")
        & (out["FECHA DE BAJA"].notna())
        & (out["FECHA DE RETIRO"].notna())
        & (out["FECHA DE BAJA"] < out["FECHA DE RETIRO"])
    )
    out.loc[mask_update_baja, "FECHA DE BAJA"] = out.loc[mask_update_baja, "FECHA DE RETIRO"]

    # 11) Recalcular DIFERENCIA_DIAS (solo VALIDAR)
    if "FECHA_INICIO_MES" in out.columns:
        mask_validar = out[col_status] == "VALIDAR"
        out.loc[mask_validar, "DIFERENCIA_DIAS"] = (
            (out.loc[mask_validar, "FECHA DE BAJA"] - out.loc[mask_validar, "FECHA_INICIO_MES"]).dt.days + 1
        )

    # 12) Retiro en mismo mes (con inasistencias temporales)
    out = _ensure_numeric_columns(out, ["DIAS_INASISTENCIA_INJUSTIFICADA_TEMP", "DIAS_PAGO_NOMINA"], default=0)

    mask_retiro_mismo_mes_inas = (
        (out[col_status] == "VALIDAR")
        & (out[col_empresa] != "SUPPLA S.A")
        & (
            (out["DIAS_PAGO_NOMINA"] + out["DIAS_INASISTENCIA_INJUSTIFICADA_TEMP"])
            == (out["FECHA DE BAJA"] - out["FECHA_INICIO_MES"]).dt.days + 1
        )
    )
    out = _set_status_obs(out, mask_retiro_mismo_mes_inas, status="OK", obs="Empleado con Retiro en Mismo Mes")

    # 13) Retiro en mismo mes (general: DIFERENCIA_DIAS == DIAS_PAGO_NOMINA)
    if "DIFERENCIA_DIAS" in out.columns:
        mask_retiro_mismo_mes = (
            (out[col_status] == "VALIDAR")
            & (out[col_empresa] != "SUPPLA S.A")
            & (out["DIFERENCIA_DIAS"] == out["DIAS_PAGO_NOMINA"])
        )
        out = _set_status_obs(out, mask_retiro_mismo_mes, status="OK", obs="Empleado con Retiro en Mismo Mes")

    # 14) Rename final
    out = rename_columns(out, {"DIAS_INASISTENCIA_INJUSTIFICADA_TEMP": "DIAS_INASISTENCIAS_TEMPORALES_MESANTERIOR"})

    return out

def apply_temporales_incapacity_rule(
    df: pd.DataFrame,
    *,
    dias_mes_actual: int,
    salario_base: float,
    col_estatus: str = "ESTATUS_DIAS",
    col_empresa: str = "EMPRESA",
    empresa_directa: str = "SUPPLA S.A",
    col_salario: str = "SALARIO MENSUAL",
    col_dias_pago: str = "DIAS_PAGO_NOMINA",
    col_obs: str = "OBSERVACION",
    col_inc_eg: str = "DIAS_INCAPACIDAD_E.G_TEMP",
    col_inc_at: str = "DIAS_INCAPACIDAD_A.T_TEMP",
) -> pd.DataFrame:
    """
    Regla: Temporales PT (empresa != SUPPLA) con incapacidad EG/AT:
    - ESTATUS_DIAS = OK
    - OBSERVACION = "<x> Días Incapacidad, Pagos al 100%"
    - DIAS_PAGO_NOMINA = dias_mes_actual

    NOTA: esta regla solo aplica cuando SALARIO MENSUAL < salario_base (tu variable salario).
    """
    if df.empty:
        return df
    ...
    # asegurar columnas
    for c in [col_estatus, col_empresa, col_salario, col_dias_pago]:
        if c not in out.columns:
            raise ValueError(f"Missing required column: {c}")

    # asegurar columnas de incapacidad
    for c in [col_inc_eg, col_inc_at]:
        if c not in out.columns:
            out[c] = 0
        out[c] = pd.to_numeric(out[c], errors="coerce").fillna(0)

    out[col_salario] = pd.to_numeric(out[col_salario], errors="coerce")

    mask = (
        (out[col_estatus] == "VALIDAR")
        & (out[col_empresa] != empresa_directa)
        & (out[col_salario] < float(salario_base))
        & ((out[col_inc_eg] > 0) | (out[col_inc_at] > 0))
    )

    suma_txt = (
        (out.loc[mask, col_inc_eg] + out.loc[mask, col_inc_at])
        .fillna(0)
        .astype(int)
        .astype(str)
        + " Días Incapacidad, Pagos al 100%"
    )

    out.loc[mask, col_estatus] = "OK"
    out.loc[mask, col_obs] = suma_txt
    out.loc[mask, col_dias_pago] = int(dias_mes_actual)
