...
def reset_index_if_named(df: pd.DataFrame, index_name: str, drop: bool = True) -> pd.DataFrame:
    """
    Si el DataFrame tiene `index_name` dentro de df.index.names, hace reset_index().
    Útil cuando groupby/apply deja la llave como índice.
    """
    if index_name in (df.index.names or []):
...
def create_sum_column(
    ...
    """
    Crea una nueva columna como suma de varias columnas.
    - Si alguna columna no existe, la crea en 0.
    - Convierte a numérico automáticamente.
    - Opcionalmente convierte a entero.
    """
    if df.empty:
        return df
    ...
    out[new_column] = out[columns_to_sum].sum(axis=1)

    if as_int:
        out[new_column] = out[new_column].astype(int)
